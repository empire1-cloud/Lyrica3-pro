...
const QualChip = ({ icon: Icon, label, value, color = "#F5C542", testId }) => (
  <div className="bg-[#0E0F17] border border-[#0E0F17] rounded-[4px] p-3 md:p-4 flex items-center gap-3"
       data-testid={testId}>
    ...
    <div className="min-w-0 flex-1">
      <div className="etched text-[#9CA3B0] text-[9px] md:text-[10px]">{label}</div>
      <div className="font-display text-[15px] md:text-[17px] text-[#F5F7FA] tracking-tight mt-0.5 truncate"
...
export default function StemDeck() {
  ...
  if (!active) {
    return <div className="p-12 text-[#9CA3B0] font-mono">Priming Empire 1 Ledger…</div>;
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 lg:p-12">
      ...
      <div className="flex items-start justify-between mb-6 md:mb-8 gap-3 flex-wrap">
        <div>
          <div className="etched text-[#9CA3B0]">// CONSOLE</div>
          <h2 className="font-display text-[28px] md:text-[40px] text-[#F5F7FA] tracking-tight leading-[1.05] mt-1">
            Sonance Pro <span className="text-[#F5C542]">Stem Deck</span>
          </h2>
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="text-right">
            <div className="etched text-[#C8CCD8]">Wallet · Sovereign</div>
            <div className="font-mono text-[12px] md:text-[13px] text-[#F5C542] mt-1">
      ...
      <div className="flex gap-2 mb-5 md:mb-6 overflow-x-auto pb-2 -mx-2 px-2 items-center" data-testid="track-ribbon">
        ...
        <button
          ...
          className="shrink-0 px-3 md:px-4 py-2 rounded-[3px] border border-[#FF2EBE]/50 bg-[#FF2EBE]/10 text-[#ffb3d5] uppercase text-[10px] md:text-[11px] tracking-[0.16em] hover:bg-[#FF2EBE]/20 transition flex items-center gap-2">
        ...
        />
        {uploadErr && <span className="shrink-0 text-[10px] text-[#FF2EBE] font-mono">{uploadErr}</span>}
          ...
          <button
            ...
            className={`shrink-0 px-3 md:px-4 py-2 rounded-[3px] border text-[10px] md:text-[11px] uppercase tracking-[0.14em] md:tracking-[0.16em] transition
              ${active.dna_tag === t.dna_tag
                ...
                : "bg-[#0E0F17] border-[#0E0F17] text-[#9CA3B0] hover:text-[#F5F7FA]"}`}>
      ...
      <div className="grid grid-cols-12 gap-4 md:gap-6">
        ...
        <div className="col-span-12 lg:col-span-8">
          <div className="panel rounded-[6px] p-4 md:p-6 mb-5 md:mb-6">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                <div className="etched text-[#9CA3B0]">Now Loaded</div>
                <h3 className="font-display text-[22px] md:text-[28px] text-[#F5F7FA] mt-1 tracking-tight">{active.title}</h3>
                <div className="text-[11px] md:text-[12px] font-mono text-[#9CA3B0] mt-1">
                  {active.cultural_matrix} · <span className="text-[#F5C542]">{active.dna_tag}</span>
                </div>
                <div className="mt-2">
              ...
              <div className="text-right">
                <div className="etched text-[#9CA3B0]">Creator</div>
                <div className="font-mono text-[12px] md:text-[13px] text-[#C8CCD8] mt-1">{active.creator}</div>
              </div>
            </div>
          </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-4 md:space-y-5" data-testid="biometrics-panel">
          <div
            className={`panel rounded-[6px] p-4 md:p-5 relative transition-all duration-300 ${pulse ? "ring-2 ring-[#FF2EBE] shadow-[0_0_30px_rgba(255,94,172,0.55)]" : ""}`}
            data-testid="biometric-card"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Fingerprint size={14} className="text-[#FF2EBE]"/>
                <span className="etched text-[#FF2EBE]">Biometrics · Human Pipes</span>
              </div>
              <span className={`text-[9px] md:text-[10px] font-mono uppercase tracking-[0.18em] ${pulse ? "text-[#FF2EBE] animate-pulse" : "text-[#6B7280]"}`}>
                {pulse ? "◉ emotional crack" : "◈ active"}
              </span>
            </div>

            <div className="space-y-2 md:space-y-3">
              <QualChip icon={Heart}     label="Vulnerability Level"  value={b.vulnerability_level || "—"} color="#FF2EBE" testId="chip-vuln"/>
              <QualChip icon={Activity}  label="Vocal Resonance"      value={b.resonance_quality   || "—"} color="#00E6FF" testId="chip-res"/>
              <QualChip icon={Sparkles}  label="Breath Profile"       value={b.breath_profile      || "—"} color="#00E6FF" testId="chip-breath"/>
              <QualChip icon={Zap}       label="Expressive Range"     value={b.expressive_range    || "—"} color="#F5C542" testId="chip-expr"/>
            </div>

            {b.signature_glyph && (
              <div className="mt-4 pt-4 border-t border-[#0E0F17]">
                <div className="etched text-[#9CA3B0] mb-1.5">Aether-Voice Signature</div>
                <div className="text-[20px] md:text-[22px] tracking-[0.3em] text-[#00E6FF]"
          ...
          <div className="panel rounded-[6px] p-4 md:p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="etched text-[#F5C542]">DNA · SynthID</span>
            </div>
            <div className="font-mono text-[11px] text-[#F5C542] break-all bg-[#0E0F17] p-3 rounded-[3px] border border-[#F5C542]/20">
              {active.dna_tag}
            </div>
            <div className="text-[10px] font-mono text-[#6B7280] mt-2 uppercase tracking-[0.18em]">
