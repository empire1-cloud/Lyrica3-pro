...
:root {
  ...
  --pink-border: rgba(255, 20, 147, 0.25);
  --purple: #a855f7;
  --gold: #ffd700;
  --green: #22c55e;
  --bg: #050508;
  ...
  --obsidian-border: rgba(17, 24, 39, 0.4);
  --text: #e0e0e0;
  --text-dim: #777777;
  --text-muted: #666666;
...
.gradient-border::before {
  ...
  border-radius: 12px;
  padding: 1px;
  background: linear-gradient(135deg, var(--pink-border), var(--obsidian-border), var(--pink-border));
...
.hero {
  ...
  min-height: 760px;
  background:
    linear-gradient(180deg, rgba(5, 5, 8, 0.74) 0%, rgba(5, 5, 8, 0.86) 48%, #050508 100%),
    linear-gradient(90deg, rgba(5, 5, 8, 0.68) 0%, rgba(5, 5, 8, 0.18) 50%, rgba(5, 5, 8, 0.72) 100%),
    url('/artists/carnival_vibe.jpg') center 42% / cover no-repeat;
}
.hero::after {
  content: '';
  position: absolute;
  inset: 0;
  background:
    radial-gradient(ellipse 680px 420px at 50% 34%, rgba(255, 20, 147, 0.2), transparent 66%),
    radial-gradient(ellipse 760px 500px at 50% 78%, rgba(0, 0, 0, 0.54), transparent 72%);
  pointer-events: none;
}
...
export default function LyricaPublicLanding({ onEnterStudio, onEnterVibe }: LandingProps) {
  return (
    <div className="lyrica-landing-root">
      ...
      <nav>
        ...
        {onEnterVibe && <button className="nav-cta vibe-btn" onClick={onEnterVibe} style={{marginRight: 8, background: 'transparent', border: '1px solid rgba(255,46,190,0.3)'}}>Vibe</button>}
