...
function getFallbackParams(vibe: string, context?: any): VibeParams {
  return {
    style: "Soul / R&B",
    mood: "Melancholic",
    tempo: 90,
    key: "B Minor",
    lyrics: `(Verse 1)\n${vibe || "In the neon glow, we find our way..."}\n\n(Chorus)\nFeel something real tonight...`,
...
export async function translateVibeToParams(vibe: string, context?: { weather?: string, time?: string, heartRate?: number }): Promise<VibeParams> {
  ...
  if (!client) {
    return getFallbackParams(vibe, context);
  }
...
export async function generateMusicStream(params: VibeParams) {
  const client = getClient();
  if (!client) return mockFallbackStream(params);
  ...
  try {
    ...
    console.warn("Soulfire model access denied or failed. Falling back to simulated stream.");
    return mockFallbackStream(params);
  }
}

async function* mockFallbackStream(params: VibeParams) {
  await new Promise(resolve => setTimeout(resolve, 1500));
  yield {
    candidates: [{
      content: {
        parts: [
          { text: params.lyrics || "Simulated lyrics...\n\n(Verse 1)\nIn the neon glow, we find our way..." }
        ]
      }
    }]
  };
